...
def pendulum_dynamics(x):
    ...
    x_dot_0 = x[0]
    x_dot_1 = x[1]
    x_dot_2 = (d - b * x_dot_1 - c * x_dot_0) / a
    return np.array([x_dot_1, x_dot_2])

def analitical_solution(x0, Tf, h):

    t_val = np.arange(0, Tf + h, h)
    x_dot_0 = x0[0] + C1 * np.exp(-25.7803 * t_val) + C2 * np.exp(-0.9697 * t_val) - 1.195
    x_dot_1 = -25.7803 * C1 * np.exp(-25.7803 * t_val) - 0.9697 * C2 * np.exp(-0.9697 * t_val)
